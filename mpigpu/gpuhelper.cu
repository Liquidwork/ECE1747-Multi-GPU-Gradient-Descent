...
__global__ void compute(double *d_data, double *params, const int *shape, double *d_mse, double *d_gradient) {
...
double gpuhelper::gradientCalculate(std::vector<double> &params, std::vector<double> &gradient) {
    double mse;
    cudaMemcpy(d_params, params.data(), h_shape[1] * sizeof(double), cudaMemcpyHostToDevice);
    compute <<< block, THREADS_PER_BLOCK, THREADS_PER_BLOCK * (h_shape[1] + 1) * sizeof(double) >>>
            (d_data, d_params, d_shape, d_partial_mse, d_partial_gradient);

    double partial_mse[block], partial_gradient[block * h_shape[1]];

    cudaMemcpy(partial_mse, d_partial_mse, block * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(partial_gradient, d_partial_gradient, block * h_shape[1] * sizeof(double), cudaMemcpyDeviceToHost);

    mse = 0;
    std::fill(gradient.begin(), gradient.end(), 0); // reset grad array
    for (int j = 0; j < block; j++){
        mse += partial_mse[j];
        for (int k = 0; k < h_shape[1]; k++){
            gradient[k] += partial_gradient[j * h_shape[1] + k];
        }
